...
var runMergeProcess = async function (change_id, project, parents, callback){
	...
	*/
	...
	getRevisionFiles(change_id, changeHead, function(result){
		...
		getTrees({changed_dirs, parents, project}, ({mtrees, ptrees})=>{
			...
			multiFetch({ urls, parser:blobParser }, ({ data }) => {
				//TODO:Merge modified blobs in advance

				// Update the bottom tree
				let {master_trees, newdirs, newObjects} = mergeBottomTree ({
					parents, 
					blobs:data,
					added_files, 
					deleted_files, 
				...
				let {treeHash, newTreeObjects} = propagateUpdate({ 
					changed_dirs,
					master_trees
				});
